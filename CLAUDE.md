...
- Tested code is non-negotiable. I'd rather have too many tests than too few.
...
(Architecture → Code Quality → Tests → Performance) with at most 4 top issues 
in each section.
...
AskUserQuestion. Also NUMBER issues and use LETTERS for options and when using 
AskUserQuestion make sure each option clearly labels the issue NUMBER and option 
LETTER so the user doesn't get confused. Make the recommended option always the 
1st option.
